...
export const Card: FC<CardProps> = ({ image, title, location, rating, reviews, description, onClick, className = '', to }) => {
  const handleClick = () => {
    if (onClick) {
      onClick();
    }
      ...
      <div className='row g-0'>
        ...
        <div className='col'>
          <div className='card-body'>
            ...
            <p className='card-text'>{description.length > 65 ? `${description.slice(0, 65)}...` : description}</p>
          </div>
        </div>
      </div>
    </div>
